...
class Conversion : public PyDukContext {};

TEST_F(Conversion, string_conversion) {
    EXPECT_EQ(context->run("'Hello, World!'"),
              boost::python::object("Hello, World!"));
}

TEST_F(Conversion, integer_conversion) {
...
TEST_F(Conversion, number_conversion) {
    EXPECT_EQ(context->run("1.2 + 2.3"),
              boost::python::object(3.5));
}

TEST_F(Conversion, boolean_conversion) {
    EXPECT_EQ(context->run("true"),
              boost::python::object(true));
}

TEST_F(Conversion, boolean_object_conversion) {
    EXPECT_EQ(context->run("new Boolean(1)"),
              boost::python::object(true));
}

TEST_F(Conversion, empty_object_conversion) {
    boost::python::dict expected;
    EXPECT_EQ(context->run("var d = {}; d;"),
              expected);
}

TEST_F(Conversion, empty_list_conversion) {
    boost::python::list expected;
    EXPECT_EQ(context->run("var d = []; d;"),
              expected);
}

TEST_F(Conversion, list_conversion) {
    boost::python::list expected;
...
TEST_F(Conversion, number_object) {
    EXPECT_EQ(context->run("new Number(12.0)"),
              boost::python::object(12));
}

TEST_F(Conversion, number_object_float) {
...
TEST_F(Conversion, simple_object_conversion) {
    boost::python::dict expected;
...
TEST_F(Conversion, complex_object_conversion) {
    boost::python::dict expected;
